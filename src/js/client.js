(function() {
	window.qad = new (class Qad {
		constructor(...args) {
			...
			Object.defineProperty(navigator, 'storageAvailable', {
				get() {
					try {
			...
			if (scheme = (navigator.storageAvailable && window.localStorage.getItem('color_scheme')))
				document.documentElement.setAttribute('data-user-color-scheme', scheme);
			if (mode = ((navigator.storageAvailable && window.localStorage.getItem('mode')) || (corsProxy && corsProxy.mode) || '').replace(/(chromeos|tablet)/, ''))
		...
		color(v) {
			let r = {},
				...
				ar = [],
				lb = [];
			let r2h = r => '#'+((1 << 24)+((+r[0]) << 16)+((+r[1]) << 8)+(+r[2])).toString(16).slice(1).split('.')[0];
			let rlr = v => {
